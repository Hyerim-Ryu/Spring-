...
<body>
			    ...
			    <div class="panel-body">
			    	<table id="boardList" class="table table-bordered table-hover">
			    		...
			    		<c:if test="${not empty mvo}">
				    		<tr>
				    			<td	colspan="5">
				    				<button onclick="goForm()" class="btn btn-danger">글쓰기</button>
				    			</td>
				    		</tr>
			    		</c:if>
						...
						<table class="table">
							...
							<tr>
								<td colspan="2" align="center">
									<button class="btn btn-danger" type="button" onclick="goInsert()">등록</button>								
									<button class="btn btn-default" type="reset" id="fclear">취소</button>
									<button onclick="goList()" href="boardList.do" class="btn btn-default">목록</button>																	
								</td>
							</tr>
						</table>
					</form>
		...
		<script type="text/javascript">
			...
			// ajax에서도 post 방식으로 데이터를 보내기위해선
			// csrf token값을 전달해야한다
			
			// token의 이름과 값을 가져오기
			// ajax에서 csrf의 이름을 사용할 때는 parameterName이 아니라 headerName사용
			...
			function makeView(data){
				...
				$.each(data, function(index, obj){
					...
					// 수정 삭제 화면
					// 조건문안에서 EL식을 쓰고 싶다면 문자열로 감싸줘야한다
					if("${mvo.memID}" == obj.memID){
						listHtml += "<br>";
						listHtml += "<span id='ub" + obj.idx + "'>";
						listHtml += "<button onclick='goUpdateForm(" + obj.idx + ")' class='btn btn-sm btn-danger'>수정화면</button></span> &nbsp;"
						listHtml += "<button onclick='goDelete(" + obj.idx + ")' class='btn btn-sm btn-default'>삭제</button> &nbsp;"
					}else {
						listHtml += "<br>";
						listHtml += "<span id='ub" + obj.idx + "'>";
						listHtml += "<button disabled onclick='goUpdateForm(" + obj.idx + ")' class='btn btn-sm btn-danger'>수정화면</button></span> &nbsp;"
						listHtml += "<button disabled onclick='goDelete(" + obj.idx + ")' class='btn btn-sm btn-default'>삭제</button> &nbsp;"
					}
			...
			function goUpdateForm(idx){
				...
				var newBtn = "<button onclick='goUpdate("+idx+")' class='btn btn-danger btn-sm'>수정</button>"
				$("#ub" + idx).html(newBtn);
			}
